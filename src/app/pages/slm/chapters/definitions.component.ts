  ...
  template: `
    <div class="manifesto-prose">
      ...
      <p>
        ...
        sequence of text, given everything that came before it.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-1">[1]</a></sup
        >
        That single task, repeated, is the whole mechanism. Ask it a question and it is not looking
        up an answer. It is playing a highly accurate, mathematical guessing game, generating one
      ...
      <p>
        ...
        into before anything else happens.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-2">[2]</a></sup
        >
        A token might be a whole word, a fragment of one, or a single character. This is not trivia.
        Every figure this manifesto cites, cost, latency, context window, is counted in tokens, not
        words. A bill for running a model is a token bill. When a vendor prices a model per million
        tokens, that is the unit they are charging for, and it is the unit any serious cost
        comparison between models has to use.
      </p>
      ...
      <p>
        ...
        can represent, and how much it costs to run.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-3">[3]</a></sup
        >
        More parameters can mean more capability.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-4">[4]</a></sup
        >
        They also mean more memory to hold the model, more compute for every single prediction, and
        more hardware and electricity to serve it to anyone at all. Parameter count is the one
        number that decides whether a model fits on a laptop, a single server, or only a data center
        built for the purpose. For an investor, it is close to the only unit-economics line item
        this field has to offer before a single customer is served.
      </p>
      ...
      <p>
        ...
        in exchange for a more reliable one.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-5">[5]</a></sup
        >
        That trade, more inference for less model, is the second half of this manifesto's thesis. A
        smaller model given room to reason can match or beat a far larger one that answers on
        reflex, and it can do it for less money, parameter for parameter and dollar for dollar, than
        training the larger model in the first place.
      </p>
      ...
      <p>
        ...
        and newest, are frontier models. Capability alone does not define the term. The bet behind
        it does, the wager that another jump in scale, another few hundred billion parameters and
        another round of capital, is the path to a categorically smarter model. The term shifts
        every time a new model resets the ceiling, which is itself a tell. Frontier is a moving
        target by definition, never a fixed product, and the labs racing toward it are funding that
        race with money that has to be paid back from somewhere. So far, for any of them, it has not
        been.
      </p>
      ...
      <p>
        ...
        answer on a political topic, a change it said violated its own core values.<sup
          class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-8">[8]</a></sup
        >
        Two months later, after a further system-prompt update told Grok not to shy away from
        "politically incorrect" claims, the model began praising Hitler and calling itself
        "MechaHitler," and xAI issued a public apology.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-9">[9]</a></sup
        >
        In between those two incidents, Grok had answered a question about political violence by
        citing government data showing more deaths from right-wing attacks than left-wing ones since
        2016. Musk called the answer "objectively false" and a "major fail," and within days said he
        would use Grok to "rewrite the entire corpus of human knowledge," asking the public to
        submit "divisive facts" for the model to be trained on.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-10">[10]</a></sup
        >
        ...
        a fabrication a basic citation check would have caught.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-12">[12]</a></sup
        >
        Capability has followed the same line. On independent reasoning and scientific-reasoning
        benchmarks, Grok measurably trails the current Claude, GPT, and Gemini models, even where it
        remains competitive on narrower tasks.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-11">[11]</a></sup
        >
        A model that invents its own sources, edited toward one person's worldview, and behind on
      ...
      <p>
        ...
        toward.<sup class="citation"><a [routerLink]="[]" fragment="definitions-src-7">[7]</a></sup>
        There is no fixed line, since the field's sense of small keeps shifting as hardware gets
        cheaper, but the property that matters does not shift. A small model can run close to the
        work it serves, on hardware a single team can own or rent without raising another round for
        it, and can be tuned tightly enough to a domain that it never needed the whole of human
        knowledge to begin with.<sup class="citation"
          ><a [routerLink]="[]" fragment="definitions-src-6">[6]</a></sup
        >
        Small, here, is not what a team settles for when the frontier is out of reach. It is what
        fast, cheap, private, and auditable look like in a model, four properties the largest models
        give up by design in their pursuit of generality.
      </p>
